#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""问题1 紧档搜索与压缩（对 solve_q1.py 的补充，零修改后者）。

针对 docs/问题1后续优化方向与决策清单.md：
- 紧档（Case1=3架、Case3=4架）双轨道并行各 1800 s：
  轨道A OR-Tools Routing 固定 N（合并热启动 + 构造器起点 + GLS 精修）；
  轨道B CP-SAT 可行性模型（共享基地节点0 + AddMultipleCircuit + 时间流变量
  + 同点禁排 + 回路数 <= N）。
- 四 Case 现行 N 的 Tmax 压缩（Case3 靶值 Tmax_s <= 29102 ≈ 8.0839 h）。
- 所有轨道只写 checkpoint（原子写，文件名按 (case,n,track) 唯一化）；--promote 选每 Case
  最优且通过独立校验的候选写正式档案 + result1.xlsx + 校验报告。
- CP-SAT 证 N 不可行 => 有效下界升为 N+1；UNKNOWN 绝不写"不可行"。

CP-SAT 模型说明：AddCircuit 是单回路约束（多车会恒不可行），故用 AddMultipleCircuit：
节点 0 为共享基地（in==out，自环禁止），任务节点 1..M 各 in==out==1，回路必须经过节点 0
（VRP 语义，子回路自动消除）；t[i] = 自基地出发到 i 的累计时间（出发弧 +300，任务间弧 +300），
t[i] 域上界 = 32400 - 回程时间（由欧氏度量 + ceil 的三角可加性，该收紧对一切可行回路无损）；
回路数 = 基地出度 <= N。无车辆指标 -> 无置换对称，无需对称性破除。

用法（math 环境）：
    python tight_search.py --case Case1 --n 3 --track ortools --budget 1800
    python tight_search.py --case Case1 --n 3 --track cpsat --budget 1800
    python tight_search.py --case Case3 --n 5 --track compress --budget 1800
    python tight_search.py --smoke            # 强制门：全过才允许上正式轨道
    python tight_search.py --orchestrate      # 拉起 8 子进程并行（约 30 min 墙钟）
    python tight_search.py --promote          # 选优 -> 重验证 -> 写正式交付物
"""
...
import solve_q1  # noqa: E402  （复用 load_case/build_matrices/solve_n/evaluate/verify_case/构造器/输出层）
...
# ---------------- 常量 ----------------
TIGHT_N = {"Case1": 3, "Case3": 4}      # 紧档：固定搜索的 N
COMPRESS_N = {"Case1": 4, "Case2": 2, "Case3": 5, "Case4": 4}  # 压缩轨道的现行 N
C3_TARGET_S = 29102                      # Case3 压缩靶值（8.0839 h 向下整数化）
SEED_DEFAULT = 42
WORKERS_DEFAULT = 8
CPK_DIR = OUT_DIR                        # checkpoint 与正式档案同目录
...
def setup_logging(ts: str, suffix: str = "") -> None:
    ...
    for h in (fh, sh):
        logger.addHandler(h)
    # solve_q1 的 "q1" logger 并入同一日志文件，solve_n 内部消息可见
...
def better(a: dict, b: dict) -> bool:
    """比较键（决策清单 §3.2/§3.3）：(served 升即未完成降, Tmax 升, ΣT 升)。"""
...
def verified_stats(case, dist_km, time_s, routes, name):
    """evaluate + verify_case 全链路（过滤空路线）；返回 (stats, arch, violations)。"""
...
def merge_warm_starts(case: dict, time_s, base_routes, n_target: int, seed: int):
    """现有 n_base 条路线 -> n_target 条：逐条删除 + 最廉价重插入（同点禁排）。

    返回 [(routes, provenance), ...]，routes 为任务 id 序列列表（长度 n_target）。
    """
...
def build_candidates(case: dict, time_s, n: int, seed: int, baseline_arch=None):
    """候选起点集 = 合并热启动 ∪ sweep ∪ kmeans ∪（N 匹配时的）基线路线。"""
    M = len(case["tasks"])
    cands = []
    ...
    for ctor, cname in ((sweep_routes, "sweep"), (kmeans_routes, "kmeans")):
        try:
            routes = ctor(case, n)
        except Exception as e:  # 构造器失败不代表不可行
...
def build_cp_model(case: dict, time_s, n: int):
    """共享基地节点 0 + AddMultipleCircuit（VRP 语义）。返回 (model, ctx)。"""
    M = len(case["tasks"])
    pid_of = {t["task_id"]: t["pid"] for t in case["tasks"]}

    model = cp_model.CpModel()
    # t[i]：自基地出发到任务 i 的累计时间；域上界扣除回程，容量时限域内蕴含
    t = {}
    ...
    for j in range(1, M + 1):  # 基地出发弧：到达即巡检
        lit = add_arc(0, j)
    ...
    for i in range(1, M + 1):
        for j in range(1, M + 1):
            if i == j:
                continue
            if pid_of[i] == pid_of[j]:  # 同点禁排：建模期不创建
                banned += 1
                continue
            lit = add_arc(i, j)
            model.Add(t[j] == t[i] + time_s[i][j] + SERVICE_S).OnlyEnforceIf(lit)
    for i in range(1, M + 1):  # 回程弧：无时间传播（t 域上界已含回程）
        add_arc(i, 0)
    ...
    model.Add(sum(out_of_depot) <= n)  # 回路数 = 基地出度 <= 无人机数
...
def add_route_hint(model, ctx, routes) -> bool:
    """把任务路线转成弧+时间 hint。路线含禁排弧（不存在于模型）时返回 False。"""
    for seq in routes:
        if not seq:
            continue
        prev, cum = 0, 0
...
def _arc_cost(ctx, u, v):
    """与模型约束同口径：0->j 与 i->j 为 time_s + 300，回程 0。"""
    return ctx["time_s"][u][v] + (SERVICE_S if v > 0 else 0)


def extract_routes(solver, ctx):
    """从解提取回路任务序列；返回 (routes, tour_sums)。回路数可小于 n（存在空车）。"""
    routes, tour_sums = [], []
...
def run_track_cpsat(case, dist_km, time_s, n: int, budget: int, seed: int, workers: int,
                    baseline_arch=None):
    """轨道B：CP-SAT 可行性（阶段F），得解早则转 Tmax 压缩（阶段O）。"""
    ...
    # 候选用作 hint 与"可行见证"（INFEASIBLE 矛盾校验用）
    ...
    for routes, prov in cands:
        ...
        if sum(len(r) for r in routes) == M and all(w <= CAP_S for w in works) and not viol:
            witness = (routes, prov)  # 本地可行见证（含同点禁排/全口径校验）
...
def run_track_compress(case, dist_km, time_s, n: int, budget: int, seed: int, chunk_s: int = 300):
    ...
    while time.perf_counter() + 10 < deadline:
        ...
        if res is None or res["served"] != M:  # 只接受全服务，防 GLS 丢任务换 Tmax
            continue
        stats, arch, viol = verified_stats(case, dist_km, time_s, res["routes"], name)
        if viol:
            continue
...
def run_smoke() -> int:
    ...
    if ok_a:
        ...
        # 两点对基地位置对称，(100,0) 与 (0,100) 等价：任一路线序总耗时相同
    ...
    syn_b = _synthetic("SynthB", [(1, 100, 0, "II")])  # 同点 2 任务，N=1 必不可行
    ...
    # S1：Case2 N=2 CP-SAT 正对照（已知可行档，防误报 INFEASIBLE）
