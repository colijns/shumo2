...
新版严格混合正式 \(M=4000\) 已完成，结果见 `question4_strict_mixed_summary.json` 和 `question4_strict_mixed_candidates.csv`：上下界夹逼违反为0；\((598,62)\) 在联合口径下可靠不足，其余7个含B候选待定，没有含B候选被认证为可靠可行。纯A保守推荐仍引用Q3的 \((619,0)\)。

本地检查点文件名含 `checkpoint`，不提交到 Git；程序采用临时文件原子替换、上一版 `.bak` 备份和损坏回退。
