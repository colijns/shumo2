"""Deterministic route replay and Q2 objective primitives."""
...
def canonical_route_signature(routes: Iterable[Iterable[int]]) -> Routes:
    """Return a sorted comparison/output signature without reordering input."""
...
def strict_key(Tmax_s: int, delta_s: int, sum_T_s: int, routes: Iterable[Iterable[int]]) -> ObjectiveKey:
    """Lexicographic Q1-style key with deterministic route tie-breaking."""
...
def balanced_key(Tmax_s: int, delta_s: int, sum_T_s: int, routes: Iterable[Iterable[int]]) -> ObjectiveKey:
    """Lexicographic Q2 key prioritizing workload spread."""
...
def epsilon_bound(Tmax_star: int, epsilon: object) -> int:
    """Return min(9h, floor((1 + epsilon) * strict optimum))."""
...
def replay_metrics(
    ...
    """Independently derive all fleet metrics from task IDs and matrices."""
