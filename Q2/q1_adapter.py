...
class ParentArchiveError(ValueError):
    """The Q1 parent archive or its source data violates the Q2 contract."""
...
def load_problem(
    ...
    *,
    ...
    """Validate and freeze one attachment/archive pair for Q2 use."""
...
def replay_archive(
    ...
    *,
    ...
    """Compatibility façade retained for the Task 1 contract tests."""
    root = _repository_root(repository_root)
    attachment = Path(attachment_path).resolve() if attachment_path else root / "attachment" / "附件1.xlsx"
    case = _load_case_at(case_name, attachment)
    distance, time_s = _immutable_matrices(case)
