...
def test_smoke_runs_real_parent_cases_without_formal_output(tmp_path):
    ...
    status = main([
        "--smoke", "--repository-root", str(root), "--output-root", str(output), "--evaluation-limit", "5",
    ])
...
def test_formal_run_verifies_epsilon_and_pareto_outputs_then_detects_tampering(tmp_path):
    ...
    assert main([
        "--repository-root", str(root), "--output-root", str(output), "--evaluation-limit", "2",
    ]) == 0
    assert main(["--verify", "--repository-root", str(root), "--output-root", str(output)]) == 0
    ...
    assert main(["--verify", "--repository-root", str(root), "--output-root", str(output)]) == 1
