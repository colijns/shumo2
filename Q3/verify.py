"""M4 independent verifier: geometric replay + nine rule checks.

Independence contract (PLAN 8.4): this module imports only the pure geometry
primitives — never safe_path/search — so the timing, waiting and feasibility
claims are checked against geometry facts, not against solver logic.
"""
...
class Replay:
    ...
    length_km: float
    flight_s: int  # integer-second duration, ceil per leg
...
def _arc(center, radius: float, theta_a: float, theta_b: float, direction: int,
    ...
    # Signed sweep along `direction`: ccw from theta_a to theta_b, or cw from
    # theta_a back to theta_b (the -1 * -X double negative in the old inline
    # expression flipped the sweep and sampled the complementary arc).
    if direction > 0:
...
def _flight_duration(points, arc_lengths: list[float]) -> int:
    """ceil per leg on the straight parts and each arc, summed."""
    total = 0
...
def replay_segment(problem, seg: SegmentRecord) -> Replay | None:
    """Rebuild the segment's geometry from path_type alone; None if the
    geometric construction itself fails (independent of the solver)."""
    p = np.asarray(_node_position(problem, seg.from_id), dtype=float)
    q = np.asarray(_node_position(problem, seg.to_id), dtype=float)
    # depart_s is the take-off instant; any wait already happened at the
    # departure node before the flight started, so the flight occupies
    # [depart_s, depart_s + duration]
...
def verify_archive(problem, archive: dict) -> list[str]:
    """Nine checks against a q3-solution-v1 archive body; [] means pass."""
    violations: list[str] = []
    task_routes = archive.get("task_routes")
    schedules = archive.get("schedules")

    # 1. served == M, task_id 1..M exactly once
    ...
    for index, route in enumerate(task_routes or []):
        # 2. every route starts and ends at the depot (implied by segment chain)
        if not route:
            violations.append(f"2: uav {index}: empty route")
        # 3. no adjacent same Point_ID
        prev_point = None
    ...
    for schedule in (schedules or []):
        # 4. per-segment replay: samples never inside an active zone
        ...
        for segment in schedule.get("segments", []):
            ...
            if replay is None:
                ...
                continue
            # time-axis continuity of the reconstructed flight: depart_s is the
            # take-off instant (any wait has already finished), so the pure
            # flight is arrive_s - depart_s
            ...
            # 5. R2: service window vs zones containing the served task
            if segment["to_id"] != 0:
            ...
            # 5. R3: waiting happens at a node outside every active disk
            if segment["wait_s"] > 0:
    ...
    # 6. timeline continuity across segments (service gaps exactly SERVICE_S)
    for schedule in (schedules or []):
    ...
    # 7. archive metrics recomputed from the schedules
    if schedules:
    ...
    # 8. archive task_routes agree with the schedule segment chains
    if task_routes:
        schedule_routes = [
    ...
    # 9. nine-hour cap when the metrology switch is on
...
def _task_routes_from_csv(problem, csv_path) -> list[list[int]]:
    """Rebuild task routes from the timetable: consecutive to_id != 0 per uav."""
    import pandas as pd
...
def _point_routes_from_workbook(problem, workbook_path, case: str) -> list[list[int]]:
    """Point_ID sequences in the result3 sheet.

    The sheet carries Point_IDs only (the submission format), not task IDs, so
    a task-level route cannot be recovered: when a UAV serves several expanded
    tasks of the same point, the search may visit them in any order (a 2-opt
    flip or block move can reorder them), and every ordering maps to the same
    Point_ID sequence. Verify at point level and keep the multiplicity check:
    each point may appear at most as many times as it has expanded tasks.
    """
    import pandas as pd
...
def verify_all(cases, config, repository_root=None) -> list[str]:
    """Read run_manifest.json and re-verify every published case end to end,
    including the cross-file consistency check (check 8) and archive hashes.

    Writes reports/verify_report.{json,md}; returns the violation list ([] =
    pass). The caller maps a nonempty list to a non-zero exit code.
    """
    ...
    if not manifest_path.is_file():
        ...
        for case in cases:
            ...
            # 8. task_routes <-> timetable CSV <-> result3.xlsx, cell by cell
            try:
