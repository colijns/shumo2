"""Attachment parsing, atomic persistence, and contract hashing for Q3."""
...
def load_problem(
    ...
    *,
    ...
    """Freeze attachment 1 through the Q2 contract, parse attachment 2 zones.

    Task expansion and ordering are bit-identical to Q2 (same attachment, same
    Q1 baseline archive); zero-duration zones are skipped with a warning.
    """
...
def load_archive(path: str | Path) -> dict[str, Any]:
    """Read a Q3 archive; JSON-decoding failures raise ValueError (fail closed)."""
    try:
...
def load_hot_start_archive(problem: ProblemData, path: str | Path) -> dict[str, Any]:
    """Load a Q2 strict archive as a warm start; fail closed on any contract drift."""
...
def atomic_write_json(path: str | Path, payload: Mapping[str, Any]) -> None:
    """Durably replace one JSON file only after a complete temporary write."""
    target = Path(path)
...
def write_archive_with_hot_start(
    ...
    *,
    ...
    """Compose the q3-solution-v1 archive (solution body + run metadata)."""
...
def write_timetable_csv(path: str | Path, solution: Solution) -> None:
    """One row per flight leg; the return leg leaves service_start/end blank."""
    rows = []
...
def write_result_workbook(path: str | Path, solutions: Mapping[str, Solution]) -> None:
    """Atomically publish original Point_ID routes in result3-compatible sheets."""
    target = Path(path)
    target.parent.mkdir(parents=True, exist_ok=True)
