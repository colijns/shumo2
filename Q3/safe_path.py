"""M2 lower layer: earliest safe travel E and service completion Phi.

Correctness-first design: every leg and every 300 s service window is checked
against the no-fly zones with closed-interval semantics; waiting only ever
happens at a node (depot or served point) that stays outside every active
safe disk (R3); any infeasible candidate fails closed with None.
"""
...
def _path_duration(path: VisibilityPath) -> int:
    """Integer-second flight time, ceil per leg (departure-relative)."""
    total = 0
...
# VisibilityPath leg-length helper (arc legs use the construction radius).
...
def _arc_bounds(path: VisibilityPath, index: int) -> tuple:
    """(theta_start, theta_end, direction, radius) of arc leg `index`, or None."""
    for leg in path.arc_legs:
        if leg.start == index:
            start, end = path.points[index], path.points[index + 1]
...
def spatiotemporal_safe(path: VisibilityPath, depart_s: float, zones) -> bool:
    """R1/R4: every leg's closed time window vs every zone's closed interval.

    Continuous entry/exit times on each leg are intersected with the integer-
    second [s_z, e_z]; endpoint contact counts as conflict (eta absorbs float
    noise).
    """
    t0 = float(depart_s)
    for i in range(len(path.points) - 1):
        ...
        for zone in zones:
            ...
            if arc is None:
                ...
                if interval is None:
                    continue
                # tangent touch leaves a ~1e-8 parameter residual (as in
                # geometry.segment_clear): a boundary graze is not a crossing.
...
def _arc_leg_conflicts(intervals, theta_start, direction, arc_angle, t0, duration, zone) -> bool:
    """Map global-angle inside-intervals onto the leg's own sweep and time."""
...
def wait_safe(problem: ProblemData, position, t0_s: float, t1_s: float) -> bool:
    """R3: the whole closed wait window at `position` stays outside every
    active safe disk."""
...
def event_delays(problem: ProblemData, zones, now_s: int) -> list[int]:
    """Event-driven wait candidates: {0} + zone-end escape times (+ SERVICE_S
    offset variant). v1 enumerates event boundaries only; correctness first.
    """
    candidates = {0}
    for zone in zones:
        if zone.end_s >= now_s:
...
def _service_conflicts(problem: ProblemData, u: Task, arrive_s: int) -> tuple[NoFlyZone, ...]:
    """R2: zones containing u whose [s_z, e_z] overlaps the 300 s service
    window [arrive_s, arrive_s + SERVICE_S] (closed intervals)."""
    conflicts = []
    for zone in problem.zones:
...
def earliest_safe_travel(problem: ProblemData, u, v, depart_s: int) -> SegmentRecord | None:
    """E_{uv}(t): earliest safe departure-arrival record, or None (fail closed).

    Candidates: direct -> wait_direct -> detour -> visibility; the best by
    (arrive_s, wait_s, distance_km) wins. A feasible direct flight is always
    the earliest, so it short-circuits.
    """
    p = (u.x_km, u.y_km)
    q = (v.x_km, v.y_km)
    zones = problem.zones
    ...
    # 2) wait_direct: wait at u (R3), then fly the straight leg.
    for zone in zones:
    ...
    # 3) detour: one disk at a time, tangent + arc + tangent, full spacetime check.
    for zone in zones:
        if zone.end_s < depart_s or zone.start_s > depart_s + direct_duration:
            continue
        interval = segment_circle_interval(
    ...
    # 4) visibility: union of disks active within the bound, full spacetime check.
...
def earliest_safe_service_completion(
    ...
    """Phi_{uv}(t): travel + 300 s service. If the service window collides with
    a zone containing u (R2), delay the departure at prev (R3: u is inside the
    active disk, waiting there is unsafe), re-solve, and retest. Returns
    (travel, arrive_s, service_end_s) or None (fail closed).
    """
...
def eval_route(problem: ProblemData, route: tuple[int, ...], uav_id: int) -> UAVSchedule | None:
    """Depart depot at t=0; Phi-fold over the route; final return leg yields S_k."""
    prev: Task | Base = Base()
    depart = 0
...
def eval_solution(problem: ProblemData, task_routes: list[tuple[int, ...]]) -> Solution | None:
    """Aggregate per-UAV routes into a Solution; None if any route is infeasible."""
...
def base_safety_assert(problem: ProblemData) -> None:
    """Hard error if the depot lies inside any disk active at t=0 (true for all
    four cases: Case2's Z3 only activates at 10:30)."""
    for zone in problem.zones:
