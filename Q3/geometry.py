"""Hand-written numpy geometry primitives (no external dependencies)."""
...
class ArcLeg:
    """One boundary arc: vertex start -> end along `direction` on the circle
    (center, radius_km). The radius is the construction radius
    (safe radius + margin), which keeps the arc strictly outside the safe disk.
    """
...
class VisibilityPath:
    """A polyline with explicit arc legs.

    points: polyline vertices; consecutive straight legs are plain segments,
    arc legs are recorded as (start_index, end_index) in arc_pairs and follow
    the union boundary (their chord would cut the disk, so geometry must be
    rebuilt from the arc pair, not from the chord). arc_legs carries the arc
    geometry for exact spatiotemporal checks.
    """
...
def segment_circle_interval(p, q, c, r) -> tuple[float, float] | None:
    """Closed t-interval in [0,1] where segment p->q meets disk (c, r).

    Returns (t0, t1) clamped to [0, 1] (t0 == t1 on a tangent touch), None when
    the segment and the disk are disjoint.
    """
    ...
    a = float(d @ d)
    if a <= 1e-18:
        return None  # zero-length leg: no interior crossing possible
    b = 2.0 * float(f @ d)
    cc = float(f @ f) - r * r
    disc = b * b - 4.0 * a * cc
    if disc < 0.0:
...
def segment_clear(p, q, disks, margin: float = 0.0) -> bool:
    """True when the open segment (p, q) stays outside every disk interior.

    A tangent touch is legal; entering a disk at either endpoint is illegal.
    """
    p, q = _vec(p), _vec(q)
    for center, radius in disks:
        ...
        if interval is None:
            continue
        t0, t1 = interval
        # tangent touch leaves a ~1e-8 parameter residual; 1e-6 covers it
        # (distance error ~1e-6 * leg length, far below the safety margin)
        if t1 - t0 > 1e-6:
            return False
    return True
...
def tangent_points(p, c, r) -> tuple[Point, Point] | None:
    """Both outer tangent touch points of point p on disk (c, r); None if inside."""
    p, c = _vec(p), _vec(c)
...
def detour_path(p, q, c, r) -> tuple[list[Point], float, int] | None:
    """Single-disk detour: p -> tangent T1 -> short arc -> tangent T2 -> q.

    Returns (points, arc_angle_rad, arc_direction) or None when either endpoint
    lies inside the disk (no tangent exists). arc_direction is +1 (ccw) or -1.
    """
    p, q, c = _vec(p), _vec(q), _vec(c)
...
def detour_distance(p, q, c, r) -> float | None:
    """Exact tangent—arc—tangent length for the short-arc detour, or None."""
...
def _normalized_intervals(lo: float, hi: float) -> list[tuple[float, float]]:
    """Split the unwrapped angle interval [lo, hi] into [0, 2pi) intervals."""
    if hi - lo <= 1e-12:
        return []
    start = lo % TAU
    span = hi - lo
...
def arc_intersection_with_circle(
    ...
    """Angle intervals (global, [0, 2pi)) of the arc that lie inside disk (c2, r2).

    The arc sweeps from theta_start along `direction` (+1 ccw / -1 cw) to
    theta_end; the short-arc distance must be <= pi. Only the arc itself is
    considered, not the full circle.
    """
    ...
    if center_distance >= r1 + r2 or center_distance <= abs(r1 - r2):
        # No circle-circle crossing: the whole arc is in or out.
    ...
    # Intersection points of the two circles; one arc between them is inside.
...
def _arc_step(start, end, direction) -> float:
    """Signed angular step from start to end along `direction` (positive)."""
    if direction > 0:
...
def visibility_path(p, q, disks, margin: float = 0.0) -> VisibilityPath | None:
    """Shortest visibility-graph path around a union of disks, or None.

    Nodes = {p, q} + per-disk tangent points from p and q + boundary crossings
    of every overlapping disk pair. Edges are straight legs that do not enter
    any disk interior plus union-boundary arcs between consecutive same-disk
    nodes. Dijkstra minimizes the exact length (segment + arc).
    """
    p, q = _vec(p), _vec(q)
    ...
    for _ in range(size):
        ...
        # boundary arcs: same-disk nodes adjacent along the union boundary.
        # The arc flies on radius + margin (strictly outside the safe disk);
        # node angles come from the safe-radius circle (same center).
...
def path_length(points) -> float:
    """Polyline length of a visibility path (km)."""
