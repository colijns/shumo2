...
def _boundary_gap_s(problem: ProblemData, segment) -> float:
    """Smallest time gap from a straight-leg disk crossing to a zone event.

    Infinity means the chord intersects no safe disk. For detour/visibility
    legs this remains a prioritization hint only; feasibility is still decided
...
def _conflict_focus_tasks(
    ...
    """Task endpoints of high-priority bottleneck/conflict segments.

    Segment priority follows the modelling tuple
    (route completion, avoidance flag, wait, detour excess, -boundary gap).
    The result is deterministic and only changes neighbourhood enumeration.
    """
